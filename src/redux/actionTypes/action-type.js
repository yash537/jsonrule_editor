export const ADD_DECISION = "ADD_DECISION";
export const REMOVE_DECISION = "REMOVE_DECISION";
export const REMOVE_DECISIONS = "REMOVE_DECISIONS";
export const UPDATE_DECISION = "UPDATE_DECISION";
export const RESET_DECISION = "RESET_DECISION";
export const ADD_ATTRIBUTE = "ADD_ATTRIBUTE";
export const RESET_ATTRIBUTE = "RESET_ATTRIBUTE";
export const REMOVE_ATTRIBUTE = "REMOVE_ATTRIBUTE";
export const UPDATE_ATTRIBUTE = "UPDATE_ATTRIBUTE";
export const UPLOAD_RULESET = "UPLOAD_RULESET";
export const ADD_RULESET = "ADD_RULESET";
export const UPDATE_RULESET_INDEX = "UPDATE_RULESET_INDEX";
export const UPDATE_NAV_STATE = "UPDATE_NAV_STATE";
export const UPDTAE_RULE_NAME = "UPDTAE_RULE_NAME";
export const FETCH_FACTS = "FETCH_FACTS";
export const ADD_FACT = "ADD_FACT";
export const UPDATE_FACT = "UPDATE_FACT";
export const FETCH_RULE_GROUPS_SUCCESS = "FETCH_RULE_GROUPS_SUCCESS";
export const FETCH_RULE_GROUPS_FAILURE = "FETCH_RULE_GROUPS_FAILURE";
export const CREATE_RULE_GROUP = "CREATE_RULE_GROUP";
export const UPDATE_RULE_GROUP = "UPDATE_RULE_GROUP";
export const FETCH_CONSTANTS = "FETCH_CONSTANTS";
export const ADD_CONSTANT = "ADD_CONSTANT";
export const UPDATE_CONSTANT = "UPDATE_CONSTANT";
export const FETCH_KEYS = "FETCH_KEYS";
export const ADD_KEY = "ADD_KEY";
export const UPDATE_KEY = "UPDATE_KEY";
export const FETCH_RULE_SUCCESS = "FETCH_RULE_SUCCESS";
export const FETCH_RULE_FAILURE = "FETCH_RULE_FAILURE";
export const CREATE_RULE = "CREATE_RULE";
export const UPDATE_RULE = "UPDATE_RULE";
export const DELETE_RULE = "DELETE_RULE";
export const DELETE_FACT = "DELETE_FACT";
export const DELETE_KEY = "DELETE_KEY";
export const DELETE_CONSTANT = "DELETE_CONSTANT";
export const DELETE_RULE_GROUP = "DELETE_RULE_GROUP";
export const RESET_RULES = "RESET_RULES";
export const SET_NOTIFICATION = "SET_NOTIFICATION";
export const FETCH_FACTS_PER_RULE = "FETCH_FACTS_PER_RULE";
export const FETCH_CONSTANTS_PER_RULE = "FETCH_CONSTANTS_PER_RULE";
export const FETCH_KEYS_PER_RULE = "FETCH_KEYS_PER_RULE";
export const DELETE_KEY_FROM_RULE = "DELETE_KEY_FROM_RULE";
export const DELETE_CONST_FROM_RULE = "DELETE_CONST_FROM_RULE";
export const FETCH_DECISION_TREE_PER_RULE = "FETCH_DECISION_TREE_PER_RULE";
export const SAVE_DECISION_TREE_PER_RULE = "SAVE_DECISION_TREE_PER_RULE";
export const DELETE_FACT_FROM_RULE = "DELETE_FACT_FROM_RULE";
export const FETCH_TYPES = "FETCH_TYPES";
export const FETCH_OPRETORS = "FETCH_OPRETORS";
export const FETCH_COMMON_FAILUER = "FETCH_COMMON_FAILUER";
export const FETCH_TREE = "FETCH_TREE";
export const FETCH_FACTS_FOR_RULE_GROUP = "FETCH_FACTS_FOR_RULE_GROUP";
export const OUTPUT_FOR_RULE_GROUP_VALIDATION =
  "OUTPUT_FOR_RULE_GROUP_VALIDATION";
export const OUTPUT_FOR_RULE_VALIDATION = "OUTPUT_FOR_RULE_VALIDATION";
export const RESET_RULE_GROUPS = "RESET_RULE_GROUPS";
