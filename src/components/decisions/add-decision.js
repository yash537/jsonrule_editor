...
class AddDecision extends Component {
  ...
  handleAdd(e) {
    ...
    if (error.formError) {
      ...
      const condition = transformTreeToRule(
        this.state.node,
        this.state.outcome,
        outcomeParams
      );
  ...
  handleTopNode(value) {
    ...
    const factsButton = this.state.factsButton.map((button) => ({
      ...button,
      disable: false
    }));
  ...
  handleActiveNode(node) {
    ...
    const facts =
      node.name === "all" || node.name === "any"
        ? parentNodeMenu
        : factsNodemenu;
    const outcomeMenus = outcomeOptions.map((option) => ({
      ...option,
      disable: false
    }));
    this.setState({
      activeNodeDepth: sortedArr,
      factsButton: facts,
      outcomeOptions: outcomeMenus
    });
  }
  ...
  handleOutputPanel(value) {
    if (value === "Add Outcome") {
      const factsOptions = this.state.factsButton.map((fact) => ({
        ...fact,
        disable: true
      }));
  ...
  fieldPanel() {
    ...
    const attribute =
      addAttribute.name &&
      attributes.find((attr) => attr.name === addAttribute.name);
    const operatorOptions = attribute && operator[attribute.type];
    const { background } = this.context;

    const placeholder =
      addAttribute.operator === "contains" ||
      addAttribute.operator === "doesNotContain"
        ? PLACEHOLDER["string"]
          ...
          {outcome.params.length > 0 &&
            outcome.params.map((param, ind) => (
              <div key={ind} className="add-field-panel">
                <InputField
                  onChange={(value) =>
                    this.handleOutputParams(value, "pkey", ind)
                  }
                  value={param.pkey}
                  label="key"
                />
                <InputField
                  onChange={(value) =>
                    this.handleOutputParams(value, "pvalue", ind)
                  }
                  value={param.pvalue}
                  label="Value"
                />
